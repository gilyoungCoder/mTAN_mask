...
def custom_softmax(scores, mask=None, dim=-2):
    """
    Compute softmax along a specific dimension, using a continuous mask for weighted averaging.

    Parameters:
    scores (torch.Tensor): 입력 스코어 텐서
    mask (torch.Tensor): 연속적인 가중치를 포함하는 마스크 텐서
    dim (int): Softmax를 적용할 차원

    Returns:
    torch.Tensor: 가중치가 적용된 Softmax 결과
    """
    # 마스크 값에 따라 가중치를 부여하기 위해 점수에 직접 곱합니다.
    if mask is not None:
        weighted_scores = scores + torch.log(mask + 1e-10)  # log(0)을 방지하기 위한 작은 상수 추가
    else:
        weighted_scores = scores
    exp_scores = torch.exp(weighted_scores - torch.max(weighted_scores, dim=dim, keepdim=True).values)
